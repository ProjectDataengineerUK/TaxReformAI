"""AT-001..AT-010 dos 4 Anexos de redução a zero, via `TestClient` + pool fake.

Nenhum destes testes toca PostgreSQL: o fake responde exatamente como o driver
(tuplas na ordem do SELECT de `buscar_reducao_zero_por_prefixo`) e guarda os
argumentos de cada query, que é como "1 query por request" vira asserção em vez
de intenção. O SQL de verdade é `test_reducao_zero_db.py`.

O seed do fake é lido das migrações 005 e 008 pelo mesmo carregador de
`test_reducao_zero_resolucao.py` — uma cópia dos 60 itens aqui seria uma
...
from tests.test_reducao_zero_resolucao import SEED

CHAVE = "chave-teste-reducao-zero"
...
class FakeCursor:
    ...
    def execute(self, sql, params=None):
        if "anexos_reducao_zero_ncm" in sql:
            self._espiao.queries_zero.append((sql, params))
...
class FakePool:
    ...
    def prefixos_consultados(self) -> list[str]:
        assert len(self.queries_zero) == 1, (
            f"esperado 1 query aos Anexos, houve {len(self.queries_zero)}"
        )
        return list(self.queries_zero[0][1][0])
...
def test_at001_cadeira_de_rodas_zera_cbs_e_ibs_citando_o_item_2_1_do_xiii(client):
    ...
    assert detalhe["reducao_zero"]["situacao"] == "APLICADA"
    assert detalhe["reducao_zero"]["anexo"] == "XIII"
    assert detalhe["reducao_zero"]["item"] == "2.1"
    assert (
        detalhe["reducao_zero"]["dispositivo_legal_ref"]
        == "LCP 214/2025, art. 145, Anexo XIII, item 2.1"
    )
    assert detalhe["reducao_zero"]["tipo_correspondencia"] == "EXATO"
    assert detalhe["reducao_zero"]["ncm_correspondido"] == "8713.10.00"
    ...
    assert corpo["reducao_zero"]["anexos_aplicados"] == ["XIII"]
...
def test_at001_o_beneficio_dispensado_e_quantificado_por_item_e_no_total(client):
    ...
    reducao = corpo["itens_detalhados"][0]["reducao_zero"]
    ...
    assert corpo["reducao_zero"]["total_cbs_dispensado"] == "9.00"
    assert corpo["reducao_zero"]["total_ibs_dispensado"] == "1.00"
    assert corpo["reducao_zero"]["itens_com_reducao_aplicada"] == 1
...
def test_ncm_pontuado_no_payload_resolve_igual_ao_sem_pontuacao(client):
    ...
    assert corpo["itens_detalhados"][0]["reducao_zero"]["item"] == "2.1"
...
def test_at002_manteiga_zera_cbs_e_ibs_citando_o_item_5_do_anexo_i(client):
    ...
    assert detalhe["reducao_zero"]["situacao"] == "APLICADA"
    assert detalhe["reducao_zero"]["item"] == "5"
    assert (
        detalhe["reducao_zero"]["dispositivo_legal_ref"]
        == "LCP 214/2025, art. 125, Anexo I, item 5"
    )
    assert detalhe["reducao_zero"]["tipo_correspondencia"] == "EXATO"
    assert detalhe["reducao_zero"]["ncm_correspondido"] == "0405.10.00"
    assert detalhe["aliquotas_aplicadas"]["cbs_percentual"] == "0"
    assert detalhe["aliquotas_aplicadas"]["ibs_percentual"] == "0"
    assert corpo["resumo_financeiro"]["total_cbs"] == "0.00"
    assert corpo["resumo_financeiro"]["total_ibs"] == "0.00"
    assert corpo["reducao_zero"]["total_cbs_dispensado"] == "9.00"
    assert corpo["reducao_zero"]["total_ibs_dispensado"] == "1.00"
    assert corpo["reducao_zero"]["anexos_aplicados"] == ["I"]
...
def test_at002_foie_gras_e_salmao_seguem_sem_receber_zero(client, ncm, item, texto):
    ...
    assert detalhe["reducao_zero"]["situacao"] == "EXCLUIDA_EXPRESSAMENTE"
    assert (detalhe["reducao_zero"]["anexo"], detalhe["reducao_zero"]["item"]) == (
        "I",
        item,
    )
    assert detalhe["reducao_zero"]["ncm_correspondido"] == texto
    assert detalhe["reducao_zero"]["tipo_correspondencia"] == "EXCECAO"
    ...
    assert corpo["reducao_zero"]["itens_com_reducao_aplicada"] == 0


def test_at002_sobreposicoes_do_anexo_i_citam_os_mesmos_itens_de_antes(client):
    """Os dois desempates já shipados, agora com a chave de 4 componentes e a
    forma nova de `itens_correspondentes` — mesmos vencedores."""
    ...
    assert formula["itens_correspondentes"] == [
        {"anexo": "I", "item": "4"},
        {"anexo": "I", "item": "26"},
    ]


# AT-003 — item fora dos QUATRO Anexos ---------------------------------------
...
def test_at003_cerveja_segue_com_a_aliquota_geral_da_fase(client):
    ...
    assert detalhe["reducao_zero"]["situacao"] == "FORA_DO_ANEXO"
    assert detalhe["reducao_zero"]["anexo"] is None
    assert detalhe["reducao_zero"]["item"] is None
    assert detalhe["reducao_zero"]["dispositivo_legal_ref"] is None
    assert detalhe["aliquotas_aplicadas"]["cbs_percentual"] == "0.900"
    ...
    assert corpo["reducao_zero"]["total_cbs_dispensado"] == "0.00"
    assert corpo["reducao_zero"]["itens_com_reducao_aplicada"] == 0
    assert corpo["reducao_zero"]["anexos_aplicados"] == []
...
def test_payload_misto_reduz_so_os_itens_dos_anexos_e_lista_quais(client):
    ...
    situacoes = [d["reducao_zero"]["situacao"] for d in corpo["itens_detalhados"]]
    ...
    assert corpo["reducao_zero"]["total_cbs_dispensado"] == "18.00"
    # Ordem da lei, não alfabética do rótulo romano.
    assert corpo["reducao_zero"]["anexos_aplicados"] == ["I", "XV"]
...
def test_anexos_aplicados_sai_na_ordem_da_lei_e_nao_alfabetica(client):
    ...
    assert corpo["reducao_zero"]["anexos_aplicados"] == ["XII", "XIII", "XV"]
...
@pytest.mark.parametrize(
    ("ncm", "anexo", "item", "texto"),
    [
        ("90213991", "XII", "5", "9021.39.91"),
        ("07108000", "XV", "2", "0710.80.00"),
    ],
)
def test_at006_protese_dentaria_e_trufa_nunca_recebem_zero(
    client, ncm, anexo, item, texto
):
    corpo = _simular(client, [_item(ncm=ncm)]).json()
    detalhe = corpo["itens_detalhados"][0]
...
def test_at008_o_exceto_descritivo_do_item_1_3_nao_bloqueia_a_reducao(client):
    ...
    assert corpo["itens_detalhados"][0]["reducao_zero"]["situacao"] == "APLICADA"
    assert corpo["resumo_financeiro"]["total_cbs"] == "0.00"
...
@pytest.mark.parametrize("ncm", ["90223000", "10061010", "07130000"])
def test_at010_vizinho_de_prefixo_nao_entra_pela_vizinhanca(client, ncm):
    corpo = _simular(client, [_item(ncm=ncm)]).json()

    assert corpo["itens_detalhados"][0]["reducao_zero"]["situacao"] == "FORA_DO_ANEXO"
    assert corpo["resumo_financeiro"]["total_cbs"] == "9.00"
...
def test_n_ncms_distintos_resolvem_em_exatamente_uma_query(client, pool):
    ...
    assert len(pool.queries_zero) == 1, "N NCMs distintos, 1 query — nunca N+1"
...
def test_ncms_repetidos_viram_um_unico_conjunto_de_prefixos(client, pool):
    ...
    assert corpo["reducao_zero"]["itens_com_reducao_aplicada"] == 50
    assert corpo["reducao_zero"]["total_cbs_dispensado"] == "450.00"  # 50 x 9,00


def test_payload_so_de_servico_nao_consulta_os_anexos(client, pool):
    corpo = _simular(client, [_item(natureza="SERVICO")]).json()

    assert pool.queries_zero == []
    assert corpo["itens_detalhados"][0]["reducao_zero"]["situacao"] == "NAO_APLICAVEL"
...
def test_payload_so_de_servico_tem_total_dispensado_zero_e_lista_vazia():
    ...
    assert corpo["reducao_zero"]["total_cbs_dispensado"] == "0.00"
    assert corpo["reducao_zero"]["itens_nao_avaliados"] == []
    assert corpo["reducao_zero"]["itens_com_reducao_aplicada"] == 0
...
def test_totais_nulos_sempre_vem_com_ao_menos_um_item_nomeado(client, pool):
    ...
    pool.falhar = True
    indisponivel = _simular(client, [_item(ncm="87131000")]).json()["reducao_zero"]

    pool.falhar = False
    ilegivel = _simular(client, [_item(ncm="0405")]).json()["reducao_zero"]
...
def test_os_dois_lookups_sao_independentes(client, pool):
    ...
    assert len(pool.queries_zero) == 1
...
def test_falha_de_conexao_degrada_para_200_com_a_aliquota_geral(client, pool, caplog):
    ...
    assert detalhe["reducao_zero"]["situacao"] == "CONSULTA_INDISPONIVEL"
    assert detalhe["aliquotas_aplicadas"]["cbs_percentual"] == "0.900"
    assert corpo["resumo_financeiro"]["total_cbs"] == "9.00"
    assert corpo["reducao_zero"]["consulta_disponivel"] is False
...
def test_indisponibilidade_anula_os_totais_e_nomeia_os_itens(client, pool):
    ...
    assert corpo["reducao_zero"]["total_cbs_dispensado"] is None
    assert corpo["reducao_zero"]["total_ibs_dispensado"] is None
    assert corpo["reducao_zero"]["itens_nao_avaliados"] == [
...
def test_um_ncm_ilegivel_anula_os_totais_mas_preserva_as_reducoes_aplicadas(client):
    ...
    assert corpo["reducao_zero"]["itens_com_reducao_aplicada"] == 1
    assert corpo["reducao_zero"]["total_cbs_dispensado"] is None
    assert [e["sku"] for e in corpo["reducao_zero"]["itens_nao_avaliados"]] == ["RUIM"]
...
def test_sem_pool_nenhum_a_feature_e_aditiva_e_nada_muda(pool):
    ...
    assert (
        corpo["itens_detalhados"][0]["reducao_zero"]["situacao"] == "CONSULTA_INDISPONIVEL"
    )
    assert corpo["resumo_financeiro"]["total_cbs"] == "9.00"
    assert corpo["reducao_zero"]["total_cbs_dispensado"] is None
...
def test_advertencia_declara_a_reducao_aplicada_e_a_condicao_nao_verificada(client):
    ...
    assert "não verifica" in corpo["reducao_zero"]["fonte_legal"]
    assert "art. 148" in corpo["reducao_zero"]["fonte_legal"]
