"""Lógica de cálculo completa de `/v1/tax/simulate`, extraída de
`api/routers/simulate.py::simular()` sem mudar comportamento (AT-007 do
DEFINE_COMPARATIVO_REGIME_ATUAL_IVA_DUAL.md) — só movida de lugar para ser
reaproveitada por `orquestracao/nos/deterministico.py`, que precisa da MESMA
paridade de cálculo (Anexos de redução, IPI, regime atual por item) que o
`/simulador` já tem, sem duplicar ~700 linhas de lógica já verificada em
produção.

Levanta `AliquotaNaoDisponivelError`/`SkuNaoResolvidoError` — nunca
`HTTPException` (acoplado ao FastAPI): cada chamador traduz para o formato
que precisa."""
...
# caminhos de cálculo, e é o único lugar do módulo que conhece o número.
...
class SkuNaoResolvidoError(Exception):
    """Substitui os 2 `HTTPException` que `simular()` levantava inline
    dentro da resolução de SKU — traduzido para 422 por CADA chamador
    (router HTTP, orquestração), nunca acoplado ao FastAPI aqui."""
...
def calcular_simulacao_completa(
    ...
    # AliquotaNaoDisponivelError propaga sem ser capturada — cada chamador
    # (router HTTP, orquestração) traduz para o formato que precisa (422 dos
    # dois lados hoje, mas nunca acoplado ao FastAPI aqui).
    ...
    regra_pis_cofins = (
        tabela_pis_cofins.buscar(regime_apuracao)
        if regime_apuracao is not None
        else None
    )
    ...
    ncms_consultar = sorted(
        {
            codigo
            for item, resolucao_sku in zip(itens, resolucoes_sku, strict=True)
    ...
    for item, resolucao_sku in zip(itens, resolucoes_sku, strict=True):
        ...
        resultado = engine.calcular(valor_base=valor_base_item, ano_operacao=ano_operacao)
        ...
        if item.natureza == "SERVICO":
            ...
            resolucao = resolver_reducao(
                item.natureza, resolucao_sku.ncm_efetivo, consulta_reducao, comprador_tipo
            )
    ...
    resposta = RespostaSimulacao(
        ...
        fonte_legal_fase=regra.fonte_legal,
    )
    return resposta
