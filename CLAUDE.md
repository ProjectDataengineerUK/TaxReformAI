...
Os 5 componentes centrais do blueprint (ingestão, motor, orquestração, API, frontend) existem agora, mais a segunda fonte de ingestão (TCU), a camada ETL real (Airflow, escrita), o CD para Cloud Run, o schema PostgreSQL real e conectado, o IPI consumindo a TIPI já ingerida, e os 10 Anexos de redução (4 a zero, 6 a 60%) zerando/reduzindo CBS/IBS por item — 12 features shipadas. A `DEPLOY_CLOUD_RUN` (2026-07-25, `.claude/sdd/archive/DEPLOY_CLOUD_RUN/`) tem **7/7 acceptance tests verificados contra infraestrutura real** — a primeira feature do projeto nesse patamar, seguida por `SCHEMA_POSTGRESQL`, `IPI_TIPI_MOTOR_CALCULO`, `REGRAS_TRIBUTARIAS_CACHE`, `ANEXOS_REDUCAO_ZERO_XII_XIII_XV` e `ANEXOS_REDUCAO_PERCENTUAL_NCM`. A aplicação está pública, funcionando, e agora persiste dados de verdade.
...
├── api/reducao.py            # ANEXOS_REDUCAO (10 Anexos: 4 a zero + 6 a 60%) — resolver_item (6 situações, + comprador_tipo) + consultar_com_seguranca + formatar_item; desempate de 6 componentes; degradação CONSERVADORA (alíquota geral, nunca null)
...
| `api/reducao.py` | Ponto de entrada dos 10 Anexos de redução (4 a zero, 6 a 60%) — `resolver_item(natureza, ncm, consulta, comprador_tipo)`, função pura com 6 situações; `EXCLUIDA_EXPRESSAMENTE` (o Anexo exclui o código) NÃO é `FORA_DO_ANEXO`. O desempate de 6 componentes é `(len(prefixo), percentual_efetivo_com_comprador, percentual_reducao_do_catálogo, -anexo_ordem, -item, -sub_item)` — o 2º componente resolve os 35 pares que citam o MESMO NCM de 8 dígitos em dois Anexos diferentes |
