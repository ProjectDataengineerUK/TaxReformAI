...
Próximo ciclo: seguir a "segunda leva" do roadmap (posição 14 — Anexos II/III/X/XI, redução de 60%
por NBS, infraestrutura de lookup inteiramente nova), depois conectar LLMs reais (Vertex AI) — 4 dos
5 nós da orquestração ainda são fake — e verificação manual em navegador, agora possível porque há
URL pública (a extensão do Chrome não estava conectada nas últimas tentativas). **SPED/IBPT** seguem
fora de escopo — ver decisão abaixo.
...
├── tests/                    # 596 testes (pytest) — os de schema/RLS/TIPI/Anexos/SKUs pulam sem DATABASE_URL, rodam de verdade no CI
