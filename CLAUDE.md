...
`contexto.md` continua sendo o blueprint completo (produto, arquitetura multi-agente, infraestrutura GCP, modelagem de dados, API e modelo de negócio) que orienta a implementação. Duas features já foram construídas seguindo o workflow SDD (`/brainstorm` → `/define` → `/design` → `/build` → `/ship`), documentadas em `.claude/sdd/`:

| Feature | Status | O que faz |
|---------|--------|-----------|
| `PIPELINE_INGESTAO_LEGAL` (`ingestion/`) | 🔄 Build completo, **ainda não shipado** — falta rodar contra GCS/Qdrant Cloud reais (aguardando `GCP_PROJECT_ID`/`GCS_BUCKET_NAME`/`QDRANT_URL`/`QDRANT_API_KEY` em `.env`) | Raspa o Planalto, estrutura a legislação em árvore AST (Lei→Título→Capítulo→Artigo→Parágrafo→Inciso→Alínea), gera chunks parent-child e indexa via embedding híbrido (BGE-M3+BM25) no Qdrant |
...
Os 5 componentes centrais do blueprint (ingestão, motor, orquestração, API, frontend) existem agora. Candidatos para o próximo ciclo: (a) resolver os blockers pendentes (credenciais GCP/Qdrant da ingestão; `langgraph` instalável) para tornar os fakes reais, (b) **schema PostgreSQL** (seção 7 — multi-tenancy real, audit log, cache de regras), já que a API/frontend atuais só têm auth mínima sem tenant real por trás, ou (c) verificação manual em navegador real (a extensão Claude in Chrome não estava conectada durante o build do frontend).
...
├── ingestion/                # PIPELINE_INGESTAO_LEGAL — scraper, parser AST, chunker, embedder, indexador Qdrant
│   ├── scraper/               # PlanaltoScraper (LegalSource)
│   ├── parser/                 # AST hierárquica (Secao recursiva + Artigo/Parágrafo/Inciso/Alínea)
...
│   └── pipeline.py                   # CLI de orquestração (executar_pipeline)
...
├── infra/terraform/         # Provisionamento do bucket GCS (validado, não aplicado)
├── tests/                    # 37 testes (pytest) cobrindo as duas features
├── .env.example              # Variáveis obrigatórias (GCP/Qdrant) — ainda não preenchidas
...
| `ingestion/pipeline.py` | Ponto de entrada do pipeline de ingestão — `executar_pipeline()` é testável com fakes; CLI real depende de `.env` preenchido |
...
| `.claude/sdd/features/*_PIPELINE_INGESTAO_LEGAL.md` + `.claude/sdd/reports/BUILD_REPORT_PIPELINE_INGESTAO_LEGAL.md` | Feature de ingestão ainda em `features/`/`reports/` (não arquivada) — `/ship` pendente de credenciais GCP/Qdrant |
...
- **Testes:** `pytest` — `python3 -m pytest tests/ -v` (37 testes, todos passando)

## Como rodar

```bash
pip install -r requirements.txt   # qdrant-client, google-cloud-storage, fastembed, typer não instaláveis neste sandbox — ver BUILD_REPORT
python3 -m pytest tests/ -v        # roda os 37 testes sem precisar de nenhuma credencial (usa fakes)
```

Para rodar o pipeline de ingestão contra infraestrutura real: preencher `.env` (copiado de `.env.example`) com `GCP_PROJECT_ID`, `GCS_BUCKET_NAME`, `QDRANT_URL`, `QDRANT_API_KEY`, rodar `terraform apply` em `infra/terraform/`, depois `python -m ingestion.pipeline run --url ... --documento-id ...`. O motor de cálculo (`motor_calculo/`) não precisa de nenhuma infraestrutura — roda direto.
...
|--------|-------------|--------|
| `@brainstorm-agent` | Explorar e refinar decisões de arquitetura ainda em aberto antes de começar a implementar | ✅ Usado nas 2 features |
| `@design-agent` | Detalhar a especificação técnica de cada componente antes do build | ✅ Usado nas 2 features |
| `@python-developer` | Implementar o motor determinístico e o pipeline de ingestão | ✅ `motor_calculo/` shipado; `ingestion/` com build completo |
| `@qdrant-specialist` | Configurar a coleção Qdrant para busca híbrida (densa BGE-M3 + esparsa BM25) | 🔄 Código pronto (`qdrant_indexer.py`), não testado contra Qdrant Cloud real |
| `@gcp-data-architect` / `@ai-data-engineer-gcp` | Implementar a infraestrutura GCP (GCS, bucket) | 🔄 Terraform validado, não aplicado (falta `GCP_PROJECT_ID`) |
...
| `@airflow-specialist` | Construir as DAGs de scraping do DOU/RFB/Comitê IBS (seção 4.2) — hoje o pipeline roda como CLI simples, não como DAG | ⏳ Não iniciado |
...
| **`@security-reviewer`** | **Sempre que houver manuseio de dados sensíveis (PII, CNPJ/CPF, multi-tenancy)** | **Já relevante** — `orquestracao/nos/classificador.py` mascara CPF/CNPJ; um bug de vazamento de PII no histórico auditável já foi encontrado e corrigido durante o build (ver `SHIPPED_2026-07-23.md` da feature) — vale uma revisão de segurança dedicada antes de conectar dados reais de usuários |
...
_Gerado por `/start` em 2026-07-22. Atualizado em 2026-07-23 após o build/ship de `MOTOR_DETERMINISTICO_CALCULO`, `ORQUESTRACAO_MULTIAGENTE`, `API_HTTP_SIMULACAO` e `FRONTEND_SIMULADOR`._
