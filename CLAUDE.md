...
Os 5 componentes centrais do blueprint (ingestão, motor, orquestração, API, frontend) existem agora, mais a segunda fonte de ingestão (TCU), a camada ETL real (Airflow, escrita), o CD para Cloud Run, o schema PostgreSQL real e conectado, o IPI consumindo a TIPI já ingerida, os 14 Anexos de redução (4 a zero por NCM, 6 a 60% por NCM, 4 a 60% por NBS) zerando/reduzindo CBS/IBS por item, e agora o piso do Anexo XVI (art. 371, 2029-2077) — 14 features shipadas. A `DEPLOY_CLOUD_RUN` (2026-07-25, `.claude/sdd/archive/DEPLOY_CLOUD_RUN/`) tem **7/7 acceptance tests verificados contra infraestrutura real** — a primeira feature do projeto nesse patamar, seguida por `SCHEMA_POSTGRESQL`, `IPI_TIPI_MOTOR_CALCULO`, `REGRAS_TRIBUTARIAS_CACHE`, `ANEXOS_REDUCAO_ZERO_XII_XIII_XV`, `ANEXOS_REDUCAO_PERCENTUAL_NCM` e `ANEXOS_REDUCAO_PERCENTUAL_NBS`. `ANEXO_XVI_PISO_ALIQUOTA_PROPRIA` é a primeira feature do projeto sem NENHUMA superfície de infraestrutura (Python puro) — nada a verificar contra Cloud SQL. A aplicação está pública, funcionando, e agora persiste dados de verdade. A `DEPLOY_CLOUD_RUN` (2026-07-25, `.claude/sdd/archive/DEPLOY_CLOUD_RUN/`) tem **7/7 acceptance tests verificados contra infraestrutura real** — a primeira feature do projeto nesse patamar, seguida por `SCHEMA_POSTGRESQL`, `IPI_TIPI_MOTOR_CALCULO`, `REGRAS_TRIBUTARIAS_CACHE`, `ANEXOS_REDUCAO_ZERO_XII_XIII_XV`, `ANEXOS_REDUCAO_PERCENTUAL_NCM` e agora `ANEXOS_REDUCAO_PERCENTUAL_NBS`. A aplicação está pública, funcionando, e agora persiste dados de verdade.
