...
| 3 | 3 | `API_EMPRESA_SKUS` | Endpoints para tenant cadastrar/listar/upload de SKUs (`empresa_skus` — schema e RLS já existem, zero rota) | ✅ Shipado 2026-08-03 (`.claude/sdd/archive/API_EMPRESA_SKUS/`) — CRUD completo + upload CSV (`/v1/tax/skus`), migração 014 (coluna `natureza`, `ncm_code` nullable), `/v1/tax/simulate` resolve `ncm`/`nbs` do catálogo quando ausentes. Primeira feature do projeto com escrita multi-tenant desde `SCHEMA_POSTGRESQL` — revisão de segurança dedicada, 5 achados (3 corrigidos, 2 registrados) |
| 4 | 5 | `LLM_REAL_VERTEX_AI` | Conectar Claude via Vertex AI de verdade (`anthropic`/`google-cloud-aiplatform` ausentes hoje) — **fundida com a posição 5 por instrução explícita do usuário** ("ja pode entregar tudo") | ✅ Shipado 2026-08-03 (`.claude/sdd/archive/LLM_REAL_VERTEX_AI/`) — verificado contra infra real (3 dispatches, 2 bugs de infra corrigidos: OOM de memória, IAM de logging); caminho 200 completo bloqueado só por quota do Vertex AI no Console do GCP (ação externa do usuário) |
| 5 | 4 | `ORQUESTRACAO_NOS_REAIS` | Tornar `classificador`/`pesquisador_legal`/`extrator_regras`/`sintetizador` reais (hoje fake), incluindo busca real no Qdrant | ✅ Entregue dentro de `LLM_REAL_VERTEX_AI` (posição 4) — não é uma feature separada, ver linha acima |
| 6 | 6 | `REMOVER_FAKE_HISTORICO` | Eliminar o vazamento de `"[FAKE]"` em `/v1/tax/query` | ✅ Resolvido como efeito colateral de `LLM_REAL_VERTEX_AI` — `sintetizador.py` não grava mais o marcador, verificado em `tests/test_nos.py`/`tests/test_api_query_llm_real.py` |
| 7 | 7 | `CLOUD_COMPOSER_PROVISIONAMENTO` | Provisionar Cloud Composer real e executar `dags/ingestao_legal_dag.py` de verdade (hoje só revisão de código) | ✅ Shipado 2026-08-04 (`.claude/sdd/archive/CLOUD_COMPOSER_PROVISIONAMENTO/`) — ciclo efêmero (provisionar → verificar → destruir, decisão explícita do usuário pelo custo real de ~US$300-400/mês); 8 bugs reais corrigidos (IAM, CLI do Airflow, sync GCS, interpolação de shell no GH Actions); bloqueado no final por instabilidade do webserver do ambiente (crash-loop por >10h após resize SMALL→MEDIUM) — DAG provada executando código real (imports OK, download do modelo iniciado) mas nunca alcançou `success`; ambiente destruído, custo zerado |
...
| 9 | 9 | `DIAGNOSTICO_BUSCA_HIBRIDA` | Root-cause do miss 4/5 no Bloco A de `ingestao.yml` (near-duplicado/boilerplate dentro da Resolução CGIBS nº 6/2026) | ✅ Já resolvido, sem ciclo dedicado — commit `89d44a1` (2026-07-25) já tinha feito o root-cause: o "miss" era sobreposição real de conteúdo entre LC 214/2025 e Resolução CGIBS 6/2026 (busca sem filtro empurrando o chunk certo pro 2º lugar por causa do gêmeo quase idêntico da outra fonte), não recuperação quebrada. Corrigido separando Bloco A (filtrado por `documento_id`, mede recuperação de verdade) de um Bloco A2 informativo (sem filtro, só registra a sobreposição). Reconfirmado com uma run real e recente de `ingestao.yml` (run `30368697093`, 2026-07-28 — POSTERIOR à criação deste roadmap): `Bloco A: 5/5`, `Bloco A2: 0/5 divergências`, `Bloco B: 2/2`, `VERIFICAÇÃO DE BUSCA HÍBRIDA: APROVADA`. Descoberto ao revisar o roadmap em 2026-08-04, antes de iniciar o `/brainstorm` |
| 10 | 10 | `BIGQUERY_DATA_WAREHOUSE` | Provisionar BigQuery para consultas analíticas em histórico de simulações (seção 5 do blueprint) | ✅ Shipado 2026-08-04 (`.claude/sdd/archive/BIGQUERY_DATA_WAREHOUSE/`) — dataset+tabela permanentes (`taxreformai_analytics.pareceres_historico`), SA dedicada mínima, sync incremental idempotente (staging+MERGE por `id`, loop por tenant via `sessao_do_tenant`, sem bypass de RLS); verificado com 2 execuções reais (82 linhas na carga inicial, zero duplicatas no rerun); primeiro cron automático de infra real do projeto — `AT-006` (disparo sozinho) pendente da passagem do horário agendado |
| 11 | 11 | `FILA_ASSINCRONA_CELERY_REDIS` | Fila assíncrona (Celery/Redis) para sustentar 50.000+ SKUs dos planos Business/Enterprise | ✅ Shipado 2026-08-04, **parcialmente bloqueado** (`.claude/sdd/archive/FILA_ASSINCRONA_CELERY_REDIS/`) — mecanismo real é Cloud Tasks (não Celery/Redis, rejeitado por exigir VPC); upload sempre assíncrono (202+job_id) comprovado real; 3 achados de infra corrigidos (IAM, delimitador de env vars, OOM 4Gi→8Gi); **meta de 50.000+ linhas NÃO alcançada** — 2 testes reais com 55.000 linhas falharam por causas distintas (OOM, depois esgotamento do pool de conexões contra o Cloud SQL `db-f1-micro`), teto revertido para 10.000 em vez de reivindicar escala não comprovada — última posição da "primeira leva", sequência completa |
