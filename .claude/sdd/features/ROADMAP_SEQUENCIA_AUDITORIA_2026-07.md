...
| 4 | 5 | `LLM_REAL_VERTEX_AI` | Conectar Claude via Vertex AI de verdade (`anthropic`/`google-cloud-aiplatform` ausentes hoje) — **fundida com a posição 5 por instrução explícita do usuário** ("ja pode entregar tudo") | 🔄 Build completo 2026-08-03 (`.claude/sdd/reports/BUILD_REPORT_LLM_REAL_VERTEX_AI.md`), revisão de segurança concluída — `/ship` pendente de verificação real via `workflow_dispatch` |
